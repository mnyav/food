...
function calc(params) {
  //Calc
  var resultCalories = document.querySelector(".calculating__result span");
  var sex,
      weight,
      height,
      age,
  ...
  function calcTotal() {
    if (!sex || !weight || !height || !age || !ratio) {
      resultCalories.textContent = "xxx";
      return;
    }
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", function (e) {
      ...
      statusMessage.src = message.louding; // showContent(message.louding);
...
function slider(_ref) {
  ...
  var back = document.querySelector(prevArrow);
  var next = document.querySelector(nextArrow);
  var current = document.querySelector(currentCounter);
  var total = document.querySelector(totalCounter);
  var slidesWrapper = document.querySelector(wrapper);
  var slidesField = document.querySelector(field);
  var width = window.getComputedStyle(slidesWrapper).width;
  var iterator = 1;
  var offset = 0;

  if (sliders.length < 10) {
    total.textContent = "0".concat(sliders.length);
    current.textContent = "0".concat(iterator);
  } else {
  ...
  var doc = document.createElement("ol");
  var dots = [];
  ...
  function stringConverter(noNumber) {
    var netNumber = noNumber.replace(/\D/g, "");
    return netNumber;
  }
...
// import exports = require("webpack");
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 2:
            ...
            throw new Error("Could not fethc ".concat(url, ", status ").concat(result.status));
